...
class NTK():
    ...
    def get_ntk_n(self, xloader, networks, recalbn=0, train_mode=False, num_batch=-1):
        ...
        for i, (inputs, _) in tqdm(enumerate(xloader)):           
            if num_batch > 0 and i >= num_batch: break
            # inputs = inputs.cuda(device=self.device, non_blocking=True)
            inputs = inputs.to(self.device)
            for net_idx, network in enumerate(networks):
                network = network.to(self.device)
                network.zero_grad()
                # inputs_ = inputs.clone().cuda(device=self.device, non_blocking=True)
                inputs_ = inputs.clone().to(self.device)

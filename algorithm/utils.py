...
def add_fc(layers, max_fc_neurons):
    layers.append({"type": "fc", "ou_c": np.random.randint(
        1, max_fc_neurons), "kernel": -1})
    return layers
...
def computeDifference(p1, p2):
    ...
    p1fc_idx = next(
        (index for (index, d) in enumerate(p1) if d["type"] == "fc"))
    p2fc_idx = next(
        (index for (index, d) in enumerate(p2) if d["type"] == "fc"))
...
def computeVelocity(gBest, pBest, p, Cg):
    ...
    if keep_all_pBest == True and keep_all_gBest == True:
        ...
        dp_fc_idx = next((index for (index, d) in enumerate(
            diff_pBest) if d["type"] == "fc" or d["type"] == "keep_fc" or d["type"] == "remove_fc"))
        dg_fc_idx = next((index for (index, d) in enumerate(
            diff_gBest) if d["type"] == "fc" or d["type"] == "keep_fc" or d["type"] == "remove_fc"))

        # Compute the velocity only between the convolution and pooling layers
        velocity.extend(velocityConvPool(
            diff_pBest[0:dp_fc_idx], diff_gBest[0:dg_fc_idx], Cg))

        # Compute the velocity between the fully connected layers
        velocity.extend(velocityFC(
            diff_pBest[dp_fc_idx:], diff_gBest[dg_fc_idx:], Cg))
...
def updateParticle(p, velocity):
    ...
    dp_fc_idx = next(
        (index for (index, d) in enumerate(p) if d["type"] == "fc"))
    dg_fc_idx = next((index for (index, d) in enumerate(
        velocity) if d["type"] == "fc" or d["type"] == "keep_fc" or d["type"] == "remove_fc"))
